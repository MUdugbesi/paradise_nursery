...
function ProductList({ onHomeClick }) {
	...
	const getTotalItemInCart = () => {
		let totalItems = 0;
		cart.forEach((plant) => (totalItems += plant.quantity));
		return totalItems;
	};
			...
			<div className='navbar' style={styleObj}>
				...
				<div style={styleObjUl}>
					...
					<div className='cart-ctn'>
						...
						<span className='cart-qtn'>{getTotalItemInCart()}</span>
					</div>
				</div>
			</div>
			...
			{!showCart ? (
				<div className='product-list-container.visible'>
					{plantsArray.map((plant) => (
						<div className='product-grid' key={plant.category}>
							<h2 className='plant_heading'>{plant.category}</h2>
							<div className='product-list' key={plant}>
